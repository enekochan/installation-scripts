...
if [ ! -e OpenSceneGraph-$VERSION ]; then
  if [ $VERSION -eq "2.8.3" ]; then
    eval "wget http://www.openscenegraph.org/downloads/stable_releases/OpenSceneGraph-2.8.3/source/OpenSceneGraph-2.8.3.zip 2>&1 | tee install.log $VERBOSE"
    eval "unzip OpenSceneGraph-2.8.3.zip 2>&1 | tee install.log $VERBOSE"
  elif [ $VERSION -eq "2.9.6" ]; then
    eval "sudo apt-get install -y subversion 2>&1 | tee install.log $VERBOSE"
    eval "svn checkout http://www.openscenegraph.org/svn/osg/OpenSceneGraph/tags/OpenSceneGraph-2.9.6 OpenSceneGraph-2.9.6 2>&1 | tee install.log $VERBOSE"
  fi
fi

cd OpenSceneGraph-$VERSION
if [ $UBUNTU_VERSION == "12.04" ];then
  eval "patch -N -p1 -i ../Patches/OpenSceneGraph/OpenSceneGraph-$VERSION-$UBUNTU_VERSION.patch 2>&1 | tee install.log $VERBOSE"
#elif [ $UBUNTU_VERSION == "10.04" -o $UBUNTU_VERSION == "10.10" -o $UBUNTU_VERSION == "11.04" -o $UBUNTU_VERSION == "11.10" ];then
  #eval "patch -N -p1 -i ../Patches/OpenSceneGraph/OpenSceneGraph-$VERSION-$UBUNTU_VERSION.patch 2>&1 | tee install.log $VERBOSE"
fi
if [ ! -e build ]; then mkdir build; fi
cd build
eval "cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS=-D__STDC_CONSTANT_MACROS 2>&1 | tee install.log $VERBOSE"
eval "echo \"Compiling OpenSceneGraph... (this may take LONG)\" 2>&1 | tee install.log"
...
if [ $DATA == "y" ]; then
  ...
  eval "sudo svn checkout http://www.openscenegraph.org/svn/osg/OpenSceneGraph-Data/tags/OpenSceneGraph-Data-2.8.0/ /usr/local/share/OpenSceneGraph/OpenSceneGraph-Data 2>&1 | tee install.log $VERBOSE"
